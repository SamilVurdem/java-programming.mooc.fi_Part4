...
@PrepareForTest({MainProgram.class, MeinCode.class})
public class MeinCodeBTest {
    ...
    public void usingObject() throws Throwable {
        ...
        MeinCode sum = createMock(MeinCode.class);
        MeinCode evenSum = createMock(MeinCode.class);
        MeinCode oddSum = createMock(MeinCode.class);
        ...
        expectNew(MeinCode.class).andReturn(sum);
        expectNew(MeinCode.class).andReturn(evenSum);
        expectNew(MeinCode.class).andReturn(oddSum);

        replay(sum, MeinCode.class);
        replay(evenSum, MeinCode.class);
        replay(oddSum, MeinCode.class);
        ...
        try {
            ...
            fail("Your program must use MeinCode type variables for calculating the sums of total, even, and odd numbers!\n"
                    ...
                    + "NB: don't add anything else than user-inputted numbers to the MeinCode objects\n"
                    + "      number -1 is used as the ending chanracter, and it should not be added to the MeinCode objects!\n"
